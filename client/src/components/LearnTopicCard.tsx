...
import { CheckCircle, Clock, Play, AlertCircle, Trophy, RotateCcw, BookOpen } from 'lucide-react';
...
const TopicCard: React.FC<TopicCardProps> = ({
  ...
  const getScorePercentage = () => {
    if (topic.score !== undefined && topic.totalQuestions) {
  ...
  return (
    ...
    >
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {topic.prerequisites.map((prereq) => {
              // This would need to be passed as props in real implementation
              const prereqMastered = true; // Placeholder
              return (
                <span
                  key={prereq}
      ...
      {/* Additional Stats for In-Progress and Mastered Topics */}
      {(status === 'in-progress' || status === 'mastered') && (
        <div className="mb-4 space-y-2">
          ...
          {topic.bestScore && (
            <div className="flex justify-between text-xs text-gray-600">
              ...
              <span className="font-medium text-green-600">{topic.bestScore}%</span>
            </div>
          )}
          {topic.lastAttempt && (
            <div className="flex justify-between text-xs text-gray-600">
              <span>Last Attempt:</span>
              <span className="font-medium">{topic.lastAttempt.toLocaleString().substring(0,10)}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex justify-between items-center">
        ...
        {!isDisabled && (
          <div className="flex space-x-2">
            {/* Learn Button - Always available for unlocked topics */}
            <button
              onClick={(e) => {
                e.stopPropagation();
                onStartLearning(topic.id);
              }}
              className="bg-indigo-100 hover:bg-indigo-200 text-indigo-700 px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-medium transition-colors flex items-center space-x-1"
            >
