...
exports.getUserAttempts = exports.createAttempt = exports.getUserStats = exports.resetTopicProgress = exports.updateTopicProgress = exports.getUserTopics = exports.initializeUserTopics = void 0;
const UserTopic_1 = require("../models/UserTopic");
const StandaredQuizAttempt_1 = require("../models/StandaredQuizAttempt");
...
const initializeUserTopics = async (req, res) => {
    try {
        ...
        await UserTopic_1.UserTopic.insertMany(userTopics);
        res.status(201).json({
...
exports.initializeUserTopics = initializeUserTopics;
const getUserTopics = async (req, res) => {
    try {
        ...
        const topics = await UserTopic_1.UserTopic.find({ userId }).sort({ createdAt: 1 });
        res.json({
            success: true,
            topics: topics
                .map(topic => ({
...
const updateTopicProgress = async (req, res) => {
    try {
        ...
        const topic = await UserTopic_1.UserTopic.findOne({ userId, topicId });
        if (!topic) {
...
const resetTopicProgress = async (req, res) => {
    try {
        const userId = req.user.id;
        const { topicId } = req.params;
        const topic = await UserTopic_1.UserTopic.findOne({ userId, topicId });
...
const getUserStats = async (req, res) => {
    try {
        const userId = req.user.id;
        const topics = await UserTopic_1.UserTopic.find({ userId });
        const stats = {
...
const createAttempt = async (req, res) => {
    try {
        const { attemptId, topicId, questions, userAnswers, score, correctAnswers, totalQuestions, timeStarted, timeCompleted, timeTaken, isRetake, originalAttemptId } = req.body;
        const userId = req.user?.id;
        // Optional: Validate input here (can use zod or express-validator)
        const newAttempt = await StandaredQuizAttempt_1.StandaredQuizAttempt.create({
            userId,
            attemptId,
            topicId,
            questions,
...
const getUserAttempts = async (req, res) => {
    try {
        ...
        const attempts = await StandaredQuizAttempt_1.StandaredQuizAttempt.find({ userId })
            .sort({ createdAt: -1 })
